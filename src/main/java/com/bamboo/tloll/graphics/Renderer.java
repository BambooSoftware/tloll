...
public final class Renderer {
	...
	public static void drawAnimatedUnit(Unit unit, int bufferId) {
		...
		//TODO: confirm this converts to float properly
		float xMin = ((float) unit.getDirection().getValue() - 1 ) / spriteBuffer.getSpriteSheetCols();
		float xMax = ((float) unit.getDirection().getValue()) / spriteBuffer.getSpriteSheetCols();
	...
	public static void drawString(GraphicsUtil gu, String currentDir, float posX, float posY, String message,
		...
		if (leftToRight) {
			...
			int bufferId = 0;
			SpriteBuffer spriteBuffer = new SpriteBuffer(gu.loadTexture(currentDir + "/Assets/Images/alphabet.png"), imageHeight, imageWidth, 1, 36);
